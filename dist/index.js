    ...
    async transform(code, id) {
      ...
      for (const importInfo of imports) {
        ...
        if (modSource.endsWith(".svg")) {
          const resolvedUrl = import_path7.default.join(import_path7.default.dirname(id), modSource);
    ...
    async resolveId(id, importer) {
      if (import_path8.default.isAbsolute(id)) {
        ...
        if (hasExtension) {
          ...
          if (await (0, import_fs_extra5.pathExists)(resolvedId)) {
          ...
          for (const extname of DEFAULT_EXTENSIONS) {
            try {
              ...
              resolvedId = normalizePath(
                import_resolve2.default.sync(withExtension, { basedir: import_path8.default.dirname(importer) })
              );
              if (await (0, import_fs_extra5.pathExists)(resolvedId)) {
                return { id: resolvedId };
              }
...
function staticMiddleware(root) {
  const serverFromRoot = (0, import_sirv.default)(root, { dev: true });
  return async (req, res, next) => {
    if (!req.url) {
      return;
    }
...
async function startDevServer() {
  ...
  app.listen(3e3, async () => {
    await optimize(root);
    console.log(
      (0, import_picocolors4.green)("\u{1F680}Server \u5DF2\u542F\u52A8"),
